...
namespace Game.Core
{
    ...
    /// 숨김/복귀(OnAnyPanelOpenChanged)와 후속 시설 화면에서의 복귀(PanelNavigationStack)를 새 코드 없이
    /// 그대로 쓴다(Docs/설계/37번 §4.1). IUIPanel.Open()이 매개변수를 받지 않으므로 카테고리마다
    /// 인스턴스를 나눈다(MonoBehaviour 아님 - HubUIWiring이 생성한다).
    /// </summary>
    public class TownCategoryPanel : IUIPanel
    {
        ...
        // UIManager.Close(PanelId)에 위임해야 한다 - 그래야 루트 depth 복귀(OnAnyPanelOpenChanged)와
        // 복귀 대상 계산(PanelNavigationStack)이 함께 처리된다.
        public void Close()
        {
